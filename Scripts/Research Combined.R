...
output_csvs = list()
input_csvs = dir(here("Data/Research_Combined"), pattern = "*.csv")
major_totals = matrix(nrow=length(input_csvs), ncol=3)

for (i in 1:length(input_csvs)) {
  output_csvs[[i]] = read.csv(here("Data/Research_Combined", input_csvs[i]))
  ...
  for(j in 1:nrow(confint)){
    confint[j, 1] = binom.confint(x=output_csvs[[i]][j, 2], n=380, method='wilson')$lower * 380
    confint[j, 2] = binom.confint(x=output_csvs[[i]][j, 2], n=380, method='wilson')$upper * 380
  }
  ...
  pdf(here("Output/Research_Combined", paste(gsub(".csv", "", input_csvs[i]), ".pdf", "")), width=10, height=8)
  p = ggplot(data=data, aes(major, value, fill=variable, label=value, alpha=factor(transparent_bars))) + geom_col(position="dodge", fill=bar_colors) + geom_text(position=position_dodge(width=0.9), hjust=1.25, size=3, color=bar_text_colors) + theme_minimal() + theme(axis.text.y = element_text(colour = label_colors)) +
    xlab("Major") + ylab("Number of Students") +
    scale_y_discrete(limits=seq(0, max(data$value), ceiling(max(data$value)/10))) +
    ...
    coord_flip() +
    ggtitle(paste(gsub("-Research Combined.csv", "", input_csvs[i]), "Research Awardees\nAUT 2017 - WIN 2019")) +
    theme(plot.title = element_text(hjust=0.5))
  plot(p)
  dev.off()
  major_totals[i, 3] = sum(output_csvs[[i]]$`Expected Count`)
  major_totals[i, 2] = sum(output_csvs[[i]]$`Actual Count`)
  major_totals[i, 1] = gsub("-Research Combined.csv", "", input_csvs[i])
}

pdf(here("Output/Research_Combined", "Total_Major_Counts-Research-Combined.pdf"), width=7, height=5)
...
for(k in 1:nrow(confint)) {
  confint[k, 1] = binom.confint(x=major_totals[k, 2], n=380, method='wilson')$lower * 380
  confint[k, 2] = binom.confint(x=major_totals[k, 2], n=380, method='wilson')$upper * 380
}
...
q = ggplot(major_totals, aes(major_totals$`Area of Study`, value, fill=variable, label=value, alpha=factor(transparent_bars))) +
  geom_col(position="dodge", fill=bar_colors) +
  geom_text(position=position_dodge(width=0.9), hjust=1.25, size=4, color=bar_text_colors) +
  theme_minimal() + theme(axis.text.y = element_text(colour = label_colors)) +
  scale_y_discrete(limits=seq(0, max(major_totals$value), 10)) +
  guides(alpha = guide_legend(reverse=TRUE)) + #theme(legend.title = element_blank()) +
  scale_fill_manual(values = bar_colors) + guides(fill=FALSE) +
  scale_alpha_manual(values=c("0.25"=0.25, "1"=1), labels=c("Expected Total", "Actual Total"), name="Overall Total = 380") +
  xlab("Area of Study") + ylab("Number of Students") +
  ggtitle(paste("Number of Research Awardees for Each Area of Study\nAUT 2017 - WIN 2019")) +
  annotation_custom(grob=textGrob(label="*Red Bars Depict", vjust=8, hjust=-3.25, gp = gpar(cex=0.6))) +
  annotation_custom(grob=textGrob(label="Underrepresented", vjust=10, hjust=-3.14, gp = gpar(cex=0.6))) +
  annotation_custom(grob=textGrob(label="Departments", vjust=12, hjust=-4.395, gp = gpar(cex=0.6))) +
